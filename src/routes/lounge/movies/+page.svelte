<script>
  ...
  import { Plus, ArrowUpDown, LayoutGrid, Table, Pencil, Trash2, Star, Flame, Minimize2, Equal, Maximize2, Circle, Play, Check, X } from '@lucide/svelte';
  import { notify } from '$lib/stores/notification.js';
  import Panel from '$lib/components/Panel.svelte';
  import MovieCard from '$lib/components/lounge/MovieCard.svelte';
  import MovieDetailModal from '$lib/components/lounge/MovieDetailModal.svelte';
  ...
  let detailMovie = $state(undefined);
  ...
  $effect(() => {
    if ($page.url.searchParams.has('open')) {
      ...
      detailMovie = movies.find(m => m.id == id) || undefined;
      goto('/lounge/movies', { replaceState: true });
    }
  });
  ...
  function loadSortMode() {
    try { return localStorage.getItem('lazarus-movies-sort') || 'default'; }
    catch { return 'default'; }
  }

  function loadFilter() {
    try { return localStorage.getItem('lazarus-movies-status-filter') || 'all'; }
    catch { return 'all'; }
  }

  function loadProps() {
    try {
      return JSON.parse(localStorage.getItem('lazarus-movies-card-props')) || { status: true, rating: true, genres: true, year: true };
    } catch { return { status: true, rating: true, genres: true, year: true }; }
  }

  function toggleProp(key) {
    visibleProps[key] = !visibleProps[key];
    localStorage.setItem('lazarus-movies-card-props', JSON.stringify(visibleProps));
  }
  ...
  let sorted = $derived.by(() => {
    ...
    if (sortMode === 'title') list.sort((a, b) => a.title.localeCompare(b.title));
      ...
      const order = { planned: 0, watching: 1, completed: 2, dropped: 3 };
  ...
  const sortLabels = { default: 'Default', updated: 'Updated', title: 'Name', status: 'Status', year: 'Year' };

  function cycleSort() {
    const order = ['default', 'updated', 'title', 'status', 'year'];
    const i = order.indexOf(sortMode);
    sortMode = order[(i + 1) % order.length];
    localStorage.setItem('lazarus-movies-sort', sortMode);
  }
  ...
  function toggleAll() {
    if (selectedItems.length === sorted.length) {
      ...
      selectedItems = sorted.map(m => m.id);
    }
  }
  ...
  async function handleBulkDelete() {
    ...
    movies = movies.filter(m => !selectedItems.includes(m.id));
    notify("Commander, " + selectedItems.length + " movies deleted.");
    selectedItems = [];
    showBulkDelete = false;
  }

  function openEdit(item) { detailMovie = item; }

  function handleSelect(item) { detailMovie = item; }
  ...
  function statusLabel(s) {
    return { planned: 'Want to Watch', watching: 'Watching', completed: 'Watched', dropped: 'Dropped' }[s] || s;
  }
</script>

<div data-section="movies-page" class="movies-page">
  <div class="page-toolbar">
    <div class="toolbar-tabs">
      ...
      <button type="button" class="tab-btn" class:active={statusFilter === 'planned'} onclick={() => setStatusFilter('planned')}><Circle size={16} /><span>Want to Watch</span></button>
      <button type="button" class="tab-btn" class:active={statusFilter === 'completed'} onclick={() => setStatusFilter('completed')}><Check size={16} /><span>Watched</span></button>
      <button type="button" class="tab-btn" class:active={statusFilter === 'dropped'} onclick={() => setStatusFilter('dropped')}><X size={16} /><span>Dropped</span></button>
      ...
      <div data-section="props-dropdown" class="props-wrap">
        ...
        {#if showProps}
          <div data-label="props-panel" class="props-panel" role="menu" onclick={(e) => e.stopPropagation()}>
            ...
            <label class="prop-item">
              <input type="checkbox" checked={visibleProps.year} onchange={() => toggleProp('year')} />
              <span>Year</span>
            </label>
          </div>
        {/if}
      </div>
      <button type="button" class="tool-btn add-btn" onclick={() => { detailMovie = null; }}>
        <Plus size={18} />
        <span>Add Movie</span>
      </button>
    </div>
  ...
  <Panel stretch={true}>
    {#if sorted.length === 0}
      <div class="empty-state" data-label="empty-state">
        <span class="empty-text">No movies yet. Add your first one.</span>
      </div>
    {:else if viewMode === 'card'}
      <div data-section="movies-grid" class="card-grid density-{cardDensity}">
        {#each sorted as item (item.id)}
          <MovieCard movie={item} {visibleProps} density={cardDensity} onedit={openEdit} ondelete={(m) => { deleteItem = m; }} onselect={handleSelect} />
        {/each}
      </div>
    {:else}
      <div data-section="movies-table" class="table-wrap">
          ...
          <tbody>
            {#each sorted as item (item.id)}
              <tr>
                ...
                <td>{item.year || '—'}</td>
                <td><span class="status-pill" style="color: {item.status === 'watching' ? 'var(--cyan)' : item.status === 'completed' ? 'var(--success)' : item.status === 'dropped' ? 'var(--danger)' : 'var(--text-dim)'}">{statusLabel(item.status)}</span></td>
                <td>
...
{#if deleteItem}
  <Modal open={true} noHeader={true} compact onclose={() => { deleteItem = null; }}>
    <DeleteConfirm title="Delete Movie" item={{ name: deleteItem.title, id: deleteItem.id }} onconfirm={(id) => handleDelete(id)} oncancel={() => { deleteItem = null; }} />
  </Modal>
{/if}

{#if showBulkDelete}
  <Modal open={true} noHeader={true} compact onclose={() => { showBulkDelete = false; }}>
    <DeleteConfirm title="Delete Selected Movies" item={{ name: selectedItems.length + ' movies', id: null }} onconfirm={() => handleBulkDelete()} oncancel={() => { showBulkDelete = false; }} />
  </Modal>
{/if}

{#if detailMovie !== undefined}
  <MovieDetailModal movie={detailMovie} {genres} onclose={handleDetailClose} />
{/if}

<style>
  .movies-page { display: flex; flex-direction: column; min-height: 0; flex: 1; }
  ...
  .props-icon { font-size: var(--fs-body); line-height: 1; }
