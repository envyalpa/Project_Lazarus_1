<script>
  ...
  let { category = null, onsave, oncancel } = $props();

  let name = $state(category?.name || '');
  let budget = $state(category?.budget ?? '');
  let icon = $state(category?.icon || 'Tag');
  let color = $state(category?.color || '--cyan');

  function submit() {
    onsave({ name, budget: Number(budget) || 0, icon, color });
  }
</script>

<div data-section="category-form" class="form-stack">
  <div class="identity-row">
    ...
    <div class="identity-content">
      ...
      <input type="text" bind:value={name} placeholder="e.g. Food, Groceries" class="field-input" />
    </div>
  </div>

  <label class="field">
    <span class="field-label">Monthly Budget</span>
    <input type="number" bind:value={budget} placeholder="0 (no limit)" class="field-input" step="0.01" />
    <span class="field-hint">Optional budget cap for this category.</span>
  </label>
...
<style>
  ...
  .field-hint { font-family: var(--font-body); font-size: var(--fs-body); color: var(--text-muted); }
