<script>
  import {
    ...
    PlusCircle, Trash2, Search, Pencil
  } from '@lucide/svelte';
  ...
  const rangeLabels = { all: 'All', day: 'Day', week: 'Week', month: 'Month', year: 'Year' };
  const rangeOrder = ['all', 'day', 'week', 'month', 'year'];
  ...
  let infoLabel = $derived.by(() => {
    ...
    if (rangeMode === 'month') return monthNames[Number(refDate.slice(5,7)) - 1] + ' ' + refDate.slice(0,4);
  ...
  async function loadTransactions() {
    ...
    if (filterType === 'entity') {
      params.set('entity', filterValue);
    } else if (filterType === 'category') {
      params.set('category_id', filterValue);
    }
  ...
  $effect(() => {
    ...
    const deps = JSON.stringify({ rangeMode, refDate, startDate, endDate });
    queueMicrotask(() => loadTransactions());
  });

  function openAdd() { showModal = true; }
  function closeModal() { showModal = false; }

  function openDelete(txn) { deleteTarget = txn; }
  ...
  function handleSelectionChange(sel) { selectedTransactions = sel; }

  function openBulkDelete() { bulkDeleteTarget = [...selectedTransactions]; }
  function closeBulkDelete() { bulkDeleteTarget = null; }
  ...
  function openBulkRename() { bulkRenameTarget = [...selectedTransactions]; bulkRenameTitle = ''; }
  function closeBulkRename() { bulkRenameTarget = null; bulkRenameTitle = ''; }
  ...
  function exitSearch() { searchActive = false; searchQuery = ''; }
  function enterSearch() { searchActive = true; }
...
<div data-section="filtered-transactions" class="ftv-container">
  {#if searchActive}
    <div bind:this={searchRef} class="search-overlay">
      <div class="search-overlay-inner">
        <div class="search-icon-wrap"><Search size={18} /></div>
        <input type="text" class="search-input" placeholder="Search transactions by title..."
          bind:value={searchQuery} onkeydown={(e) => { if (e.key === 'Escape') exitSearch(); }} />
        <button type="button" class="search-close" onclick={exitSearch}><X size={18} /></button>
      </div>
    </div>
  {:else}
    <div class="toolbar">
      <button type="button" class="btn-add" onclick={openAdd}><Plus size={18} /> Add Transaction</button>
      ...
      <div class="toolbar-right">
        <div class="nav-group">
          <button type="button" class="tb-btn" onclick={cycleRange}>
        ...
        {#if rangeMode !== 'all'}
          ...
          <div class="range-group">
            <label class="range-field"><DatePicker value={startDate} onchange={(v) => { startDate = v; }} /></label>
            <span class="range-sep">—</span>
            <label class="range-field"><DatePicker value={endDate} onchange={(v) => { endDate = v; }} /></label>
          </div>
          <div class="toolbar-divider"></div>
        {/if}

        <button type="button" class="tb-btn search-btn" onclick={enterSearch}><Search size={16} /></button>
      </div>
    </div>
  {/if}

  <TransactionsTable {transactions} {categories} {accounts} {people}
    ondelete={openDelete} onupdate={handleUpdate} onselectionchange={handleSelectionChange} />
</div>
  ...
  <div data-section="modal-backdrop" class="backdrop" role="presentation" onclick={handleBackdrop}>
    <div data-section="modal" class="modal" role="dialog" aria-modal="true">
      <div data-label="modal-header" class="modal-header">
        <h3 data-label="modal-title" class="modal-header-title"><PlusCircle size={20} /> New Transaction</h3>
...
{#if bulkRenameTarget}
  <div data-section="bulk-rename-backdrop" class="backdrop" role="presentation" onclick={(e) => { if (e.target === e.currentTarget) closeBulkRename(); }}>
    <div data-section="modal" class="modal modal-delete" role="dialog" aria-modal="true">
      <div data-label="modal-header" class="modal-header">
        <h3 data-label="modal-title" class="modal-header-title"><Pencil size={20} /> Rename {bulkRenameTarget.length} Transactions</h3>
        <button type="button" class="close-btn" onclick={closeBulkRename}><X size={18} /></button>
      </div>
      <div data-label="modal-body" class="modal-body">
...
<style>
  ...
  .search-overlay-inner { display: flex; align-items: center; gap: 12px; width: 100%; }
  .search-icon-wrap { display: flex; align-items: center; color: var(--text-dim); flex-shrink: 0; }
  .search-input {
    flex: 1; padding: 10px 14px; background: var(--bg-card); border: 1px solid var(--border);
    border-radius: var(--radius); font-family: var(--font-body); font-size: var(--fs-body);
    color: var(--text); outline: none; transition: border-color 0.2s;
  }
  .search-input:focus { border-color: var(--cyan); box-shadow: 0 0 0 2px rgba(0, 212, 255, 0.12); }
  .search-input::placeholder { color: var(--text-muted); }
  .search-close {
    display: flex; align-items: center; justify-content: center; width: 32px; height: 32px;
    background: none; border: 1px solid var(--border); border-radius: var(--radius);
    color: var(--text-dim); cursor: pointer; transition: all 0.2s; flex-shrink: 0;
  }
  .search-close:hover { color: var(--text); background: var(--bg-elevated); border-color: var(--cyan-dim); }
  .toolbar {
    display: flex; align-items: center; gap: 0; padding: 15px 30px;
    background: var(--bg-surface); border-top: 1px solid var(--border-glow);
    border-bottom: 1px solid var(--border-glow); box-shadow: 0 1px 6px var(--cyan-glow);
    margin: 0 -20px 10px -20px; flex-wrap: wrap;
  }
  ...
  .btn-add {
    ...
    font-size: var(--fs-body); font-weight: 600; padding: 8px 18px; background: transparent;
    color: var(--cyan); border: 1px solid var(--cyan); border-radius: var(--radius);
    cursor: pointer; transition: all 0.2s; text-transform: uppercase; letter-spacing: 0.5px;
    white-space: nowrap; height: 38px; box-sizing: border-box;
  }
  .btn-add:hover { background: rgba(0, 212, 255, 0.1); box-shadow: 0 0 12px var(--cyan-glow); }
  .btn-bulk-delete {
    display: flex; align-items: center; gap: 6px; font-family: var(--font-body);
    font-size: var(--fs-body); font-weight: 600; padding: 8px 18px; background: rgba(239, 68, 68, 0.08);
    color: var(--danger); border: 1px solid var(--danger); border-radius: var(--radius);
    cursor: pointer; transition: all 0.2s; text-transform: uppercase; letter-spacing: 0.5px;
    white-space: nowrap; height: 38px; box-sizing: border-box; margin-left: 8px;
  }
  .btn-bulk-delete:hover { background: rgba(239, 68, 68, 0.15); box-shadow: 0 0 12px rgba(239, 68, 68, 0.2); }
  .btn-bulk-rename {
    display: flex; align-items: center; gap: 6px; font-family: var(--font-body);
    font-size: var(--fs-body); font-weight: 600; padding: 8px 18px; background: rgba(0, 212, 255, 0.06);
    color: var(--cyan); border: 1px solid var(--cyan-dim); border-radius: var(--radius);
    cursor: pointer; transition: all 0.2s; text-transform: uppercase; letter-spacing: 0.5px;
    white-space: nowrap; height: 38px; box-sizing: border-box; margin-left: 8px;
  }
  .btn-bulk-rename:hover { background: rgba(0, 212, 255, 0.12); box-shadow: 0 0 12px var(--cyan-glow); }
  .nav-group { display: flex; align-items: center; gap: 4px; }
  .tb-btn {
    display: flex; align-items: center; gap: 4px; padding: 8px 10px; height: 38px;
    box-sizing: border-box; background: var(--bg-card); border: 1px solid var(--border);
    border-radius: var(--radius); color: var(--text-dim); font-family: var(--font-body);
    font-size: var(--fs-body); font-weight: 600; cursor: pointer; transition: all 0.15s; white-space: nowrap;
  }
  ...
  .tb-info {
    ...
    padding: 8px 14px; height: 38px; box-sizing: border-box; background: var(--bg-card);
    border: 1px solid var(--border); border-radius: var(--radius); color: var(--text);
    font-family: var(--font-body); font-size: var(--fs-body); font-weight: 500;
    cursor: pointer; transition: all 0.15s; min-width: 120px; text-align: center; line-height: 1;
  }
  .tb-info:hover { border-color: var(--cyan-dim); color: var(--cyan); }
  .range-group { display: flex; align-items: center; gap: 6px; }
  .range-field { display: flex; }
  .range-field :global(.trigger-btn) { height: 38px !important; padding: 8px 10px !important; font-size: var(--fs-body) !important; box-sizing: border-box !important; }
  .range-sep { color: var(--text-muted); font-size: var(--fs-body); }
  ...
  .modal { background: var(--modal-bg); border: 1px solid var(--border); border-radius: var(--radius); max-width: 1290px; width: 100%; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 0 30px var(--cyan-glow); }
  ...
  .modal-header-title { display: flex; align-items: center; gap: 10px; font-family: var(--font-heading-1); font-size: var(--fs-heading-2); font-weight: 700; color: var(--cyan-dim); text-transform: uppercase; letter-spacing: 1px; margin: 0; }
  ...
  .rename-modal-body { display: flex; flex-direction: column; gap: 16px; }
  .rename-field { display: flex; flex-direction: column; gap: 6px; }
  .field-label { font-family: var(--font-heading-1); font-size: var(--fs-heading-2); font-weight: 600; color: var(--cyan); text-transform: uppercase; letter-spacing: 1px; }
  .rename-input { width: 100%; box-sizing: border-box; padding: 10px 12px; background: var(--bg-surface); border: 1px solid var(--border); border-radius: 6px; font-family: var(--font-body); font-size: var(--fs-body); color: var(--text); transition: all 0.2s; }
  .rename-input:focus { outline: none; border-color: var(--cyan); box-shadow: 0 0 0 2px rgba(0, 212, 255, 0.15); }
  ...
  .ftv-container { flex: 1; display: flex; flex-direction: column; min-height: 0; overflow: hidden; }
</style>
