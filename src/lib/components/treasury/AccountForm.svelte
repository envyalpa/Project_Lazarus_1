<script>
  ...
  let { account = null, onsave, oncancel } = $props();

  let name = $state(account?.name || '');
  let type = $state(account?.type || 'bank');
  let currency = $state(account?.currency || 'INR');
  let icon = $state(account?.icon || 'Wallet');
  let color = $state(account?.color || '--cyan');
  let is_asset = $state(account?.is_asset ?? 1);
  let show_in_summary = $state(account?.show_in_summary ?? 1);
  let total_payable = $state(account?.total_payable || 0);

  function submit() {
    onsave({ name, type, currency, icon, color, is_asset: is_asset ? 1 : 0, show_in_summary: show_in_summary ? 1 : 0, total_payable: total_payable || 0 });
  }
</script>

<div data-section="account-form" class="form-stack">
  <div class="identity-row">
    ...
    <div class="identity-content">
      ...
      <input type="text" bind:value={name} placeholder="e.g. Federal Bank Debit" class="field-input" />
    </div>
  </div>

  <label class="field">
    <span class="field-label">Type</span>
    <select bind:value={type} class="field-input">
  ...
  <label class="field">
    <span class="field-label">Asset / Liability</span>
    <div class="checkbox-row">
      <label class="checkbox-label" class:active={is_asset === 1}>
        <input type="radio" bind:group={is_asset} value={1} /> Asset
      </label>
      <label class="checkbox-label" class:active={is_asset === 0}>
        <input type="radio" bind:group={is_asset} value={0} /> Liability
      </label>
    </div>
  </label>

  <label class="field" style={is_asset ? 'opacity:0.4;pointer-events:none' : ''}>
    <span class="field-label">Total Payable (₹)</span>
    <input type="number" class="field-input" bind:value={total_payable} placeholder="0" min="0" step="0.01" />
  </label>
  ...
  <label class="field">
    ...
    <select bind:value={currency} class="field-input">
