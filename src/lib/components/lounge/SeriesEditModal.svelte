<script>
  ...
  import { BookMarked, Check, Trash2, X } from '@lucide/svelte';
  import { colorValues } from '$lib/shared/colors.js';

  let { series = null, onsave, oncancel, ondelete } = $props();

  let name = $state(series?.name || '');
  let icon = $state(series?.icon || 'BookMarked');
  let color = $state(series?.color || '--cyan');
  ...
  function handleSave() {
    if (!name?.trim()) return;
    onsave({ id: series?.id || null, name: name.trim(), icon, color, description });
  }
</script>

<Modal open={true} noHeader={true} compact onclose={oncancel}>
  <div data-section="series-edit-modal" class="series-modal-inner">
    <div class="modal-header">
      <h3 class="modal-header-title"><BookMarked size={18} /> {series ? 'Edit Series' : 'Add Series'}</h3>
    ...
    <div class="modal-body">
      <div class="modal-layout">
        <div class="image-preview" style="border-color: {hexColor(color)};">
          <DynamicIcon name={icon} size={48} color={hexColor(color)} />
        </div>
        <div class="form-column">
          <label class="field-group">
            ...
            <div class="icon-color-name-row">
              ...
              <input type="text" bind:value={name} placeholder="Series name" class="field-input" style="flex: 1;" />
            </div>
          </label>
          <div class="field-group description">
            ...
            {#if mode === 'edit'}
              <textarea bind:value={description} placeholder="Series description" class="field-textarea"></textarea>
            {:else}
      ...
      <div class="form-actions">
        {#if series}
          <button type="button" class="delete-btn" onclick={() => { ondelete?.(series); }}><Trash2 size={15} /> Delete</button>
        {/if}
        <div class="right-actions">
          <button type="button" class="cancel-btn" onclick={oncancel}>Cancel</button>
          <button type="button" class="save-btn" onclick={handleSave} disabled={!name.trim()}><Check size={15} /> {series ? 'Accept' : 'Create'}</button>
        </div>
      </div>
    </div>
  </div>
...
<style>
  .series-modal-inner { display: flex; flex-direction: column; min-width: 540px; }
  ...
  .image-preview { width: 100px; flex-shrink: 0; border-radius: 6px; border: 2px solid; background: rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center; min-height: 100px; }
  .form-column { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 14px; }
