<script>
  ...
  let { seasons = [], onedit, ondelete, onincrement, ondecrement, onmarkall, onadd, stretch = false, headerRight = null, loading = false } = $props();

  let lockedIds = $derived.by(() => {
    const locked = new Set();
    let prevComplete = true;
    for (const s of seasons) {
      if (!prevComplete) locked.add(s.id);
      prevComplete = s.total_episodes > 0 && s.episodes_watched >= s.total_episodes;
    }
    return locked;
  });
</script>
