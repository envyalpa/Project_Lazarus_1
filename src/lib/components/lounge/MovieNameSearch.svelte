  ...
  async function handleSelect(r) {
    ...
    try {
      const res = await fetch('/api/scrape/movie', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'resolve-result', url: r.url || (r.tmdb_id ? `https://www.themoviedb.org/movie/${r.tmdb_id}` : ''), title: r.title, imdb_id: r.imdb_id, cover_url: r.cover_url })
      });
...
<div class="name-row">
  <input
    ...
    placeholder="Or search by movie name..."
