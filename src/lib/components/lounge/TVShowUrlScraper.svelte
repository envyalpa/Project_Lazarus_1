<script>
  ...
  import TVShowNameSearch from './TVShowNameSearch.svelte';
  ...
  async function handleFetch() {
    ...
    try {
      ...
      if (!data.title && !data.cover_url) {
        throw new Error('Could not extract TV show details from this URL');
      }
      scraped = true;
      onscraped?.(data);
    } catch (e) {
...
<div data-section="url-scraper" class="scraper-box">
  <div class="url-row">
    <input
      type="url"
      bind:value={url}
      placeholder="Paste a TV show URL..."
  ...
  <TVShowNameSearch {onscraped} {renderResultsExternal} {onsearchresults} {onsearchloading} {onsearcherror} />
</div>
