<script>
  import { ArrowLeft, X, RefreshCw, Tv, Trash2, Save, Search, Plus, Layers, Pencil } from '@lucide/svelte';
  import { slide } from 'svelte/transition';
  import { notify } from '$lib/stores/notification.js';
  import Panel from '$lib/components/Panel.svelte';
  import Modal from '$lib/components/operations/Modal.svelte';
  import TVShowDetailForm from '$lib/components/lounge/TVShowDetailForm.svelte';
  import TVShowSeasonsPanel from '$lib/components/lounge/TVShowSeasonsPanel.svelte';
  import TVShowUrlScraper from '$lib/components/lounge/TVShowUrlScraper.svelte';
  import TVShowHeroWrapper from '$lib/components/lounge/TVShowHeroWrapper.svelte';
  ...
  let { show: initialShow, genres = [], onclose } = $props();
  let show = $state(initialShow);
  let showId = $derived(show?.id);
  let seasons = $state([]);
  ...
  let mode = $derived(show ? 'edit' : 'create');
  ...
  async function loadSeasons() {
    seasonsLoading = true;
    const res = await fetch(`/lounge/tv/${showId}/seasons`);
  ...
  async function handleDeleteShow() {
    ...
    await fetch(`/lounge/tv/${showId}`, { method: 'DELETE' });
    deleteItem = null; onclose?.();
    notify("Commander, TV show deleted: " + (title || ''));
  }

  async function handleSaveSeason(data) {
    if (editSeason) {
      await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'update', id: editSeason.id, ...data }) });
      notify("Commander, season updated for " + (show?.title || ''));
    } else {
      await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create', ...data }) });
      notify("Commander, season added to " + (show?.title || ''));
    }
    await loadSeasons(); showSeasonModal = false; editSeason = null;
  }

  async function handleIncrement(id) {
    await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'increment', id }) });
    await loadSeasons();
  }

  async function handleDecrement(id) {
    await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'decrement', id }) });
    await loadSeasons();
  }

  async function handleMarkAll(id) {
    await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'mark-all', id }) });
    await loadSeasons();
  }

  async function handleDeleteSeason(season) {
    await fetch(`/lounge/tv/${showId}/seasons`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete', id: season.id }) });
    notify("Commander, season removed from " + (show?.title || ''));
    await loadSeasons();
  }
  ...
  async function handleAcceptSync() {
    ...
    for (const s of toCreate) {
      await fetch(`/lounge/tv/${showId}/seasons`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'create', season_number: s.season_number, total_episodes: s.total_episodes, episodes_watched: 0 })
      });
    }
    notify("Commander, " + toCreate.length + " seasons synced to " + (show?.title || ''));
    syncOpen = false;
    syncSeasons = [];
    await loadSeasons();
  }

  function handleFormSaved(savedShow) {
    show = savedShow;
    if (mode === 'create') {
      onclose?.();
    }
...
<Modal open={true} full={mode !== 'create'} narrow={mode === 'create'} noHeader={true} noBodyScroll={true} onclose={onclose}>
  <div data-section="tv-detail-modal" class="detail-modal-body">
    {#if mode === 'create' || show}
      <div class="detail-grid">
        ...
        {#snippet overviewHeaderRight()}
          ...
          {/if}
          {#if mode !== 'create'}
            <button type="button" class="panel-header-btn del-btn" onclick={() => { deleteItem = show; }} title="Delete">
        ...
        <div class="left-col">
          {#if mode === 'edit' && show && !heroDismissed}
            <TVShowHeroWrapper {show} {seasons} onincrement={handleIncrement} onedit={() => { heroDismissed = true; }} />
          {/if}
          {#if mode === 'create'}
            <Panel title="Auto-Fetch" icon={Search}>
              <TVShowUrlScraper onscraped={handleScraped}
          ...
          {/if}
          <Panel title={mode === 'create' ? 'Add TV Show' : 'Edit TV Show'} icon={Tv} stretch={true} class="form-panel" headerLeft={overviewHeaderLeft} headerRight={overviewHeaderRight}>
            <TVShowDetailForm show={show} genres={localGenres}
          ...
          {#if mode === 'create'}
            <button type="button" class="create-save-btn" onclick={async () => { if (saveFn) await saveFn(); }} disabled={!formDirty}>
              <Plus size={20} /> Add TV Show
            </button>
          {/if}
          {#if mode === 'edit'}
            <div transition:slide={{ duration: 250 }}>
              <Panel title="Auto-Fetch" icon={Search}>
                <TVShowUrlScraper onscraped={handleScraped} initialUrl={show?.source_url || ''} />
              </Panel>
            </div>
          {/if}
        </div>
        ...
        <div class="right-col">
              ...
              <div class="xray-container">
                ...
                <div class="xray-face" class:show={rightColView === 'results'}>
                  <div class="xray-scroll">
                    ...
                    {:else}
                      <div class="search-status">Search for a TV show above to see results here.</div>
                    {/if}
                    </div>
                  </div>
                </div>
          ...
          {:else}
            {#snippet seasonsHeaderRight()}
              <button type="button" class="panel-header-btn sync-btn" onclick={handleSyncSeasons} title="Sync seasons from TMDB">
                <RefreshCw size={16} />
              </button>
            ...
            <div class="seasons-panel-wrap">
              <TVShowSeasonsPanel {seasons} stretch={true}
    ...
    {:else}
      <div class="loading">Loading TV show data...</div>
    {/if}
  </div>
</Modal>
...
{/if}
{#if syncOpen}
  <Modal open={true} title="Sync Seasons from TMDB" onclose={() => { syncOpen = false; syncSeasons = []; }}>
    <div data-section="sync-seasons-modal" class="sync-modal-body">
      {#if syncLoading}
        <div class="sync-status">Looking up seasons on TMDB...</div>
      {:else if syncSeasons.length === 0}
        <div class="sync-status">No seasons found for this title.</div>
      {:else}
        <p class="sync-info">Detected seasons for <strong>{show?.title}</strong>:</p>
...
{#if deleteItem}
  <Modal open={true} noHeader={true} compact onclose={() => { deleteItem = null; }}>
    <DeleteConfirm title="Delete TV Show" item={{ name: deleteItem.title, id: deleteItem.id }} onconfirm={handleDeleteShow} oncancel={() => { deleteItem = null; }} />
  </Modal>
{/if}
