<script>
  ...
  let { show = null, genres = [], ondirty, onsaveset, onsaved, oncancel, scrapedData = null, scrapedSeasons = [], manualSeasons = [] } = $props();
  ...
  function pristineFromProps() {
    return {
      ...
      status: show?.status || 'not_started',
  ...
  $effect(() => {
    ...
    editSynopsis = show?.synopsis || '';
    editCoverUrl = show?.cover_url || '';
    editStatus = show?.status || 'not_started';
    editRating = show?.rating || null;
    editGenreIds = show?.genres?.map(g => g.id) || [];
  ...
  $effect(() => {
    if (scrapedData && scrapedData !== appliedScrapedRef) {
      ...
      const d = scrapedData;
      console.log('[TVForm] scrapedData received:', { title: d.title, genre_ids: d.genre_ids, new_genres: d.new_genres?.length });
      ...
      if (d.genre_ids?.length) {
        ...
        console.log('[TVForm] editGenreIds after merge:', editGenreIds);
      }
      covers = d.covers || [];
      coverIndex = 0;
    }
  ...
  async function handleSave() {
    ...
    console.log('[TVForm] handleSave payload:', { title: payload.title, genre_ids: payload.genre_ids });
    if (show) {
      const res = await fetch(`/lounge/tv/${show.id}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        lastDirty = false;
        ondirty?.(false);
        notify("Commander, TV show updated: " + updated.title);
      ...
      if (res.ok) {
        ...
        onsaved?.(created);
        notify("Commander, TV show created: " + created.title);
      } else if (res.status === 409) {
        const err = await res.json();
        notify(err.error);
      } else {
        notify("Error: Failed to create TV show.");
      }
    }
  }
...
<div class="edit-form-grid" data-section="tv-detail-form">
  <div class="gc-col1">
  ...
  <div class="gc-title-synopsis">
    <label class="efield">
      ...
      <input type="text" bind:value={editTitle} class="efield-input" placeholder="TV show title" />
    </label>

    <label class="efield synopsis-field">
      <span class="efield-label">Synopsis</span>
      <textarea bind:value={editSynopsis} class="efield-input etextarea" placeholder="TV show synopsis..."></textarea>
    </label>
  </div>
