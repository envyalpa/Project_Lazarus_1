...
class AgentStore {
  ...
  async processNewBook(bookId, title, author) {
    this.open = true;
    const cardIdx = this.messages.length;
    this.messages.push({
      sender: 'edi',
      ...
      message: 'Correcting metadata and generating synopsis...',
      animIdx: this.getNextCardAnim()
    });
    this.saveHistory();
    ...
    try {
      const res = await fetch('/api/agent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'process_book', rawTitle: title, rawAuthor: author || '', charLimit: 600 })
      });
      ...
      if (res.ok) {
        ...
        if (data.success && data.correction) {
          ...
          const updatePayload = {};
          if (c.title) updatePayload.title = c.title;
          if (c.author) updatePayload.author = c.author;
          if (c.series_id != null) updatePayload.series_id = c.series_id;
          if (c.volume_number != null) updatePayload.volume_number = c.volume_number;
          if (c.total_volumes != null) updatePayload.total_volumes = c.total_volumes;
          if (c.genres?.length) updatePayload.genre_ids = c.genres;
          if (c.synopsis) updatePayload.synopsis = c.synopsis;

          if (Object.keys(updatePayload).length > 0) {
            await fetch(`/lounge/books/${bookId}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
        ...
        this.messages[cardIdx] = {
          ...
          statusText: 'Complete',
          taskLabel: 'Complete',
          message: `Finished processing "${title}". Corrections saved.`
        };
      } else {
        ...
        this.messages[cardIdx] = {
          ...
          statusText: 'Failed',
          taskLabel: 'Failed',
          message: `Error: API returned ${res.status}. ${errText}`
        };
      }
    } catch (err) {
      console.error(err);
